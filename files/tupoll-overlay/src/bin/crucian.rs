...
pub fn compile_aggregated_context() -> String {
    ...
    combined_code
      ...
      ("gui-apps/crucian/files/crucian/src/ai.rs", r#"use serde::{Deserialize, Serialize};
...
/// Универсальный асинхронный вызов Ollama (возвращает чистый String кода)
pub async fn ai_completion_call(
    ...
    let (url, model_name, authorization) = if use_local_ollama {
        ("http://localhost:11434/v1/chat/completions".to_string(), "qwen2.5-coder:1.5b".to_string(), None)
    } else {
    ...
    match request.send().await {
        Ok(resp) => {
            if !resp.status().is_success() {
                return format!("// ❌ Ошибка сервера ИИ ({})", resp.status());
            }
            match resp.json::<OpenAICompatibleResponse>().await {
                Ok(ai_data) => {
                    if let Some(choice) = ai_data.choices.first() {
                        let mut code = choice.message.content.clone();
                        // Вычищаем markdown-теги, если модель случайно их добавила
                        if code.starts_with("```") {
...
pub fn update_line_numbers(_file_buffer: &gtk4::TextBuffer, _line_numbers_buffer: &gtk4::TextBuffer) {
}
...
fn build_ui(app: &Application) {
    ...
    // 🔥 НАШ НОВЫЙ ИНДИКАТОР: Окошко-лейбл для вывода текущей строки кода
    let line_indicator_label = Label::builder()
        .label("Ln: 1")
        .xalign(0.0)
        .css_classes(vec!["line-indicator"])
        .build();
    ...
    // Аккуратно упаковываем его в тулбар левой панели
    header_box.append(&model_dropdown);
    header_box.append(&status_label);
    header_box.append(&line_indicator_label); 
...
// 🔥 ВСТАВЛЯЕМ СВЯЗКУ КНОПОК И КРАБА СЮДА:
    ...
    let lines_buf_clone = lines_buffer.clone();
              ...
              // 🔥 СИНХРОНИЗАЦИЯ ОКНА-ИНДИКАТОРА С ТЕКУЩЕЙ СТРОКОЙ
    let _line_lbl_clone = line_indicator_label.clone();

            // =========================================================================
    // 🔥 МОНОЛИТНЫЙ ЗАМОК ДЛЯ ИСКЛЮЧЕНИЯ ЛЮБЫХ ЧУДЕС С BACKSPACE И КУРСОРOM
    // =========================================================================
    
    // 1. Принудительно гасим варнинг неиспользуемой очереди (если нужно для совместимости)
    let _ = SHARED_CONTEXT_QUEUE.read();

    // 2. Сигнал удержания каретки и обновления золотого окошка Ln:
    file_buffer.connect_closure(
        "mark-set",
        false,
        gtk4::glib::closure_local!(
            #[watch] line_indicator_label,
            move |_buf: gtk4::TextBuffer, iter: gtk4::TextIter, mark: gtk4::TextMark| {
                // Нас интересует строго каретка реального ввода пользователя
                if mark.name().as_deref() == Some("insert") {
                    
                    // ЕСЛИ В ЭТОТ МИГ ИДЕТ ПОКРАСКА СИНТАКСИСА — ИГНОРИРУЕМ СИС-МУСОР GTK4!
                    ...
                    // Бронебойный Unix-фильтр: Записываем строку только если она больше нуля
                    if current_line > 0 {
                        let _ = std::fs::write("/var/tmp/wm/crucian_cursor", format!("{}", current_line));
                        line_indicator_label.set_label(&format!("Ln: {}", current_line + 1));
                    } else {
                        // Подстраховка на самой первой строчке документа
                        let _ = std::fs::write("/var/tmp/wm/crucian_cursor", "0");
                        line_indicator_label.set_label("Ln: 1");
                    }
                }
            }
        ),
       ...
       // 3. Основной сигнал изменения текста (Исправленные системные имена)
    let _file_view_clone = file_view.clone();
...
let handler_id = Arc::new(RwLock::new(None::<SignalHandlerId>));

             // --- 9. АСИНХРОННЫЙ КОНВЕЙЕР ИИ (ГЛОБАЛЬНЫЙ ПЕРЕХВАТ Ctrl+Enter ДЛЯ ЗАЩИТЫ ВЫДЕЛЕНИЯ) ---
    let chat_text_buffer = buffer.clone();          
    let current_file_buffer = file_view.buffer();   
    
    // Создаем клавиатурный контроллер глобально для главного окна приложения
    ...
    let chat_buf_clone = chat_text_buffer.clone();            
    let file_buf_clone = current_file_buffer.clone();         
    let status_clone = status_label.clone();                  
    let line_lbl_clone = line_indicator_label.clone(); 
    let file_view_clone = file_view.clone();                  

    window_key_controller.connect_key_pressed(move |_, keyval, _, modifier| {
        // Проверяем комбинацию Ctrl + Enter
        if (keyval == gtk4::gdk::Key::Return || keyval == gtk4::gdk::Key::KP_Enter) 
            && modifier.contains(gtk4::gdk::ModifierType::CONTROL_MASK) 
        {
            // 🔥 ЖЕСТКОЕ УСЛОВИЕ: Реагируем ТОЛЬКО если пользователь нажал Ctrl+Enter, 
            // находясь внутри левой строки ввода промпта! 
            if prompt_clone.is_focus() {
                let user_question = prompt_clone.text().to_string();
                if user_question.trim().is_empty() {
                    return gtk4::glib::Propagation::Stop;
                }

                let insert_mark = file_buf_clone.get_insert();
                let current_line = file_buf_clone.iter_at_mark(&insert_mark).line();

                prompt_clone.set_text("");
                status_clone.set_label("ИИ работает...");
                line_lbl_clone.set_label(&format!("Ln: {} [ИИ...]", current_line + 1));

                let mut status_iter = chat_buf_clone.end_iter();
                chat_buf_clone.insert_with_tags_by_name(&mut status_iter, &format!("\nUser: {}\n", user_question), &["bold"]);

                // Считываем пути контекста из SQLite базы
                let aggregated_code = db::compile_aggregated_context();
                
                let b_chat = chat_buf_clone.clone();  
                let b_lower = file_buf_clone.clone(); 
                let s_label = status_clone.clone();
                let l_label = line_lbl_clone.clone();
                let v_file = file_view_clone.clone(); 

                gtk4::glib::MainContext::default().spawn_local(async move {
                    let handle = AI_RT.spawn(async move {
                        let final_prompt = format!(
                            "Контекст проекта:\n\n{}\n\nЗадание: Допиши код в месте разрыва. Ответь чистым кодом без markdown тегов.\nВопрос пользователя: {}",
                            aggregated_code, user_question
                        );
                        ai::ai_completion_call(final_prompt, true, None).await
                    });

                    let result = match handle.await {
                        Ok(res) => res,
                        Err(_) => "// ❌ Ошибка фонового потока ИИ".to_string(),
                    };

                    s_label.set_label("Готов");
                    
                    let fresh_line = b_lower.iter_at_mark(&b_lower.get_insert()).line();
                    l_label.set_label(&format!("Ln: {}", fresh_line + 1));

                    // Вставляем сгенерированный чистый код прямо под текущий курсор
                    b_lower.insert_at_cursor(&result);

                    let mut ai_iter = b_chat.end_iter();
                    b_chat.insert(&mut ai_iter, "AI: Код успешно внедрен под курсор.\n");

                    let mut current_cursor = b_lower.iter_at_mark(&b_lower.get_insert());
                    v_file.scroll_to_iter(&mut current_cursor, 0.0, false, 0.0, 0.0);
                });

                // 🔥 НАМЕРТВО УБИВАЕМ СИГНАЛ: На самом верхнем уровне окна гасим Ctrl+Enter.
                // Никакие встроенные сигналы виджета Entry или TextView теперь не сработают.
                // Выделение в file_view гарантированно спасено!
                return gtk4::glib::Propagation::Stop; 
            }
        }
     ...
     win.add_controller(window_key_controller);
...
file_buffer.tag_table().add(&TextTag::builder().name("syntax_bracket").foreground("#808080").build());
       // 🔥 АСИНХРОННЫЙ IDLE-ТАЙМЕР СИНТАКСИСА (БРОНЕБОЙНАЯ ЗАЩИТА BACKSPACE)
    let file_view_clone = file_view.clone();
    let daemon_id_clone = handler_id.clone(); 

    // Ссылочный контейнер для хранения ID текущего таймера ожидания
    let current_timeout_id = std::sync::Arc::new(parking_lot::RwLock::new(None::<gtk4::glib::SourceId>));

    let handler_signal_id = file_buffer.connect_changed(move |buf| {
        let buf_clone = buf.clone();
        let view_clone = file_view_clone.clone();
        let d_id_clone = daemon_id_clone.clone();
        let timeout_id_clone = current_timeout_id.clone();

        // 1. Если пользователь продолжает быстро нажимать клавиши или Backspace —
        // сбрасываем предыдущий таймер, не давая синтаксису ворваться во время ввода!
        if let Some(old_id) = timeout_id_clone.write().take() {
            old_id.remove();
        }

        // 2. Взводим новый чистый таймер на 180 миллисекунд
        let new_id = gtk4::glib::timeout_add_local(std::time::Duration::from_millis(180), move || {
            // Этот код сработает ТОЛЬКО когда пользователь сделал микро-паузу вводе!
            let ext_str: Option<&str> = None; 
            ...
            // Очищаем контейнер таймера после успешного выполнения
            if let Some(mut lock) = timeout_id_clone.try_write() {
                *lock = None;
            }

            // Завершаем работу таймера (вызывается строго один раз)
            gtk4::glib::ControlFlow::Break
        });

        // Запоминаем ID запущенного таймера
        *current_timeout_id.write() = Some(new_id);
    });

    // Фиксируем основной сигнал в вашем родном handler_id
    *handler_id.write() = Some(handler_signal_id);
                    ...
                    if let Some(path) = file.path() {
                        ...
                        if let Ok(content) = std::fs::read_to_string(&std_path) {
                            ...
                            // 4. Запоминаем путь в переменной редактора
                             let mut p_lock = path_dialog_clone.write(); 
...
pub fn run_syntax_daemon(
    ...
    file_extension: Option<&str>
     ...
     // 🔥 ШАГ 1: ЧИТАЕМ СТРОКУ И КОЛОНКУ ИЗ ФАЙЛА-ПАМЯТИ
    let (mut saved_line, mut saved_column) = (0, 0);
    if let Ok(coords) = fs::read_to_string(CURSOR_FILE) {
        let parts: Vec<&str> = coords.trim().split(':').collect();
        if parts.len() == 2 {
            saved_line = parts[0].parse::<i32>().unwrap_or(0);
            saved_column = parts[1].parse::<i32>().unwrap_or(0);
        }
    }

    // Подстраховка на случай пустого файла памяти
    if saved_line == 0 && saved_column == 0 {
        let insert_mark = buffer.get_insert();
        let current_iter = buffer.iter_at_mark(&insert_mark);
        saved_line = current_iter.line();
        saved_column = current_iter.line_offset();
    }

    // Вычисляем видимый экран (Viewport)
    let rect = view.visible_rect();
    let (start_visible, _) = view.line_at_y(rect.y());
    
    let mut start_line = start_visible.line() - 20;
    if start_line < 0 { start_line = 0; }
    
    let mut end_line = start_visible.line() + (rect.height() / 15) + 20; 
    if end_line > buffer.line_count() { end_line = buffer.line_count(); }

    let start_all = buffer.iter_at_line(start_line).unwrap_or_else(|| buffer.start_iter());
    let end_all = buffer.iter_at_line(end_line).unwrap_or_else(|| buffer.end_iter());
    
    // Стираем теги только в видимом диапазоне
    ...
    let mut iter = start_all.clone();
    let mut expect_function_name = false;

    while iter < end_all && !iter.is_end() {
        if iter.starts_word() {
    ...
    let mut string_iter = start_all.clone();
    while string_iter < end_all && !string_iter.is_end() {
    ...
    let mut search_iter = start_all.clone();
    while let Some((comment_start, _)) = search_iter.forward_search(comment_marker, gtk4::TextSearchFlags::VISIBLE_ONLY, Some(&end_all)) {
    ...
    // 🔥 ШАГ 2: СИЛОВОЕ ВОССТАНАВЛИВАЕМ КУРСОР ТОЧНО В КООРДИНАТУ [СТРОКА : КОЛОНКА]
    let mut reset_cursor_iter = buffer.iter_at_line(saved_line).unwrap_or_else(|| buffer.start_iter());
    
    // Сдвигаем итератор по строке вправо на сохраненное количество символов
    for _ in 0..saved_column {
        if reset_cursor_iter.ends_line() || !reset_cursor_iter.forward_char() {
            break;
        }
    }
    
    // Намертво фиксируем каретку на её законном месте
    buffer.place_cursor(&reset_cursor_iter);

    // 3. УДЕРЖИВАЕМ ВЬЮПОРТ ЭКРАНА НА МЕСТЕ
    view.scroll_to_iter(&mut reset_cursor_iter, 0.0, false, 0.0, 0.0);
