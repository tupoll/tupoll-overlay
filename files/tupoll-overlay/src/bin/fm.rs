...
fn main() -> glib::ExitCode {
    ...
    // ЗАПУСКАЕМ МОНИТОР ТАК, ЧТОБЫ ОН ТОЧНО РАБОТАЛ
    std::thread::spawn(|| {
        println!(">>> [DEBUG] Поток zbus стартовал"); // Если этой строки нет — поток не создался
    ...
    app.run() // Всё, что ниже этой строки, НЕ ВЫПОЛНИТСЯ до закрытия окна
}
...
fn main() -> glib::ExitCode {
    // 1. ПИНАЕМ ДЕМОН ПЕРЕД ЗАПУСКОМ
    // Это заставит udisks2 проснуться после перезагрузки
    let _ = Command::new("udisksctl").arg("status").output();
...
fn build_ui(app: &Application) {
    ...
    provider.load_from_data("
        ...
        .usb-mode { border: 1px solid #89b4fa; } /* Синяя рамка для флешек */
    ");
                ...
                if path != *last {
                    *last = path.clone();
                    ...
                    if let Ok(info) = file.query_info("standard::*", gio::FileQueryInfoFlags::NONE, None::<&gio::Cancellable>) {
                        ...
                        // ПРОВЕРКА: Если путь начинается на /run/media, это флешка!
                        if path.starts_with("/run/media") {
                            name_c.set_markup(&format!("🚀 <b>{}</b>", info.display_name()));
                            win_c.add_css_class("usb-mode");
                        } else {
                            name_c.set_text(&info.display_name());
                            win_c.remove_css_class("usb-mode");
                        }
