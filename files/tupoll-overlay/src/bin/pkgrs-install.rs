...
fn main() -> std::io::Result<()> {
    ...
    ("sys-apps/pkgrs/files/src/bin/crab.rs", r#"use std::{thread, io::{self, Write}, time::Duration};
...
pkg_postinst() {
    ...
    cp -rv /usr/bin/python-exec-2 /usr/lib/python-exec/python-exec-2
...
[[bin]]
name = "writer-font"
path = "src/bin/font.rs"

[[bin]]
name = "pkgrs"
path = "src/bin/pkgrs.rs"
...
[[bin]]
name = "lmerge"
path = "src/bin/lmerge.rs"

[[bin]]
name = "pkgrs-use-cache"
path = "src/bin/pkgrs-use-cache.rs"

[[bin]]
name = "pkgrs-dispatch-setup"
path = "src/bin/dispatch_setup.rs"
...
debug = false
incremental = true "#),
    ("sys-apps/pkgrs/files/src/etc-update.fish", r#"#!/bin/fish
# =========================================================================
# REPLACEMENT FOR ETC-UPDATE IN PURE FISH (pkgrs ecosystem)
# PROTECTION STATUS: STRICT MONASTERY MODE ACTIVE
# =========================================================================

set LOG_FILE "/var/log/pkgrs-etc-update.log"
echo (set_color green)"[>] Инициализация реактивного etc-update на Fish..."(set_color normal)

set cfg_updates (find /etc -name "._cfg*" -type f 2>/dev/null)
if test (count $cfg_updates) -eq 0
    echo (set_color -o green)"[SYSTEM OK]"(set_color normal)" Нет доступных обновлений конфигурации."
    exit 0
end

for update_file in $cfg_updates
    set dir_name (dirname $update_file)
    set base_name (basename $update_file)
    set orig_name (string replace -r '^\._cfg[0-9]+_' '' $base_name)
    set target_config "$dir_name/$orig_name"

    # --- ОБОРОННЫЙ БЛОК: ЗАПРЕЩАЕМ ТРОГАТЬ МОЁ ---
    if string match -q "*pkgrs*" $target_config
        or string match -q "*pinnacle*" $target_config
        or string match -q "*/etc/portage/package.use*" $target_config
        or string match -q "*/etc/init.d/*" $target_config
        
        echo (set_color -o red)"[🛡️ ETC BLOCKED]"(set_color normal)" Попытка изменения священного конфига: $target_config" >> $LOG_FILE
        echo (set_color yellow)"  [-] Ликвидация суррогата: $update_file"(set_color normal)
        rm -f $update_file 
        continue
    end

    # --- ИНТЕРАКТИВНЫЙ РЕЖИМ ДЛЯ ОСТАЛЬНЫХ СИСТЕМНЫХ ФАЙЛОВ ---
    echo ""
    echo (set_color cyan)"Обнаружено системное обновление для: $target_config"(set_color normal)
    diff -u $target_config $update_file

    echo ""
    echo "Выберите действие для "(basename $target_config)":"
    echo "  1) Заменить старый конфиг новым"
    echo "  2) Выкинуть обновление (оставить всё как есть)"
    echo "  3) Пропустить этот файл"
    
    read -p "set_color green; echo -n 'Ввод [1-3]: '; set_color normal" user_choice
    switch $user_choice
        case 1
            echo "[+] Обновление применилось к $target_config"
            mv -f $update_file $target_config
        case 2
            echo "[-] Обновление уничтожено."
            rm -f $update_file
        case '*'
            echo "[>] Пропущено."
    end
end
echo (set_color green)"[ГОТОВО] Все доступные системные файлы обработаны."(set_color normal) "#),
    ("sys-apps/pkgrs/files/src/dispatch-conf.fish", r#"#!/bin/fish
# =========================================================================
# REPLACEMENT FOR DISPATCH-CONF IN PURE FISH (pkgrs ecosystem)
# ANTI-AUTO-MERGE DEFENSE: ACTIVE (MONASTERY MODE)
# =========================================================================

set LOG_FILE "/var/log/pkgrs-dispatch-conf.log"
...
for update_file in $cfg_updates
    ...
    # --- ЖЕСТКИЙ ОБОРОННЫЙ БЛОК ---
    if string match -q "*rust-portage-watcher*" $target_config
        or string match -q "*pkgrs*" $target_config
        or string match -q "*pinnacle*" $target_config
        or string match -q "*/etc/init.d/*" $target_config
        or string match -q "*/etc/portage/package.use*" $target_config
        
        echo (set_color -o red)"[🛡️ DISPATCH BLOCKED]"(set_color normal)" Попытка авто-замены: $target_config" >> $LOG_FILE
        rm -f $update_file
        continue
    end
...
inotifywait -q -m -r -e close_write,move,create,delete "$WATCH_PATH" | while read -r line; do
 ...
 "#),
    ...
    //          PKGRS-BIN                                |
    //---------------------------------------------------
    ("sys-apps/pkgrs/files/src/bin/dispatch_setup.rs", r##"// src/bin/dispatch_setup.rs
use std::fs;
use std::path::Path;
...
fn main() -> std::io::Result<()> {
    ...
    if unsafe { libc::getuid() } != 0 {
        eprintln!(" \x1B[1;31m[!] Ошибка:\x1B[0m Запускайте через sudo.");
        std::process::exit(1);
    }

    let base_path = Path::new("/var/db/repos/tupoll-overlay/");   
    
    // ВАШ ФИРМЕННЫЙ МАССИВ СТРУКТУРЫ ОВЕРЛЕЯ (Теперь с обоими скриптами)
    let text_files = [
        // 1. ЗАЩИТНЫЙ СКРИПТ ДЛЯ DISPATCH-CONF
        ("sys-apps/pkgrs/files/dispatch-conf.fish", r#"#!/bin/fish
# =========================================================================
# REPLACEMENT FOR DISPATCH-CONF IN PURE FISH (pkgrs ecosystem)
# ANTI-AUTO-MERGE DEFENSE: ACTIVE (MONASTERY MODE)
# =========================================================================
...
"#),
        ...
        // 2. ЗАЩИТНЫЙ СКРИПТ ДЛЯ ETC-UPDATE
        ("sys-apps/pkgrs/files/etc-update.fish", r#"#!/bin/fish
# =========================================================================
# REPLACEMENT FOR ETC-UPDATE IN PURE FISH (pkgrs ecosystem)
# PROTECTION STATUS: STRICT MONASTERY MODE ACTIVE
# =========================================================================
...
"#),
    ...
    // 1. АВТОМАТИЧЕСКАЯ ЗАПИСЬ СТРУКТУРЫ В GIT-ОВЕРЛЕЙ
    println!("--- Генерация структуры оверлея... ---");
    for (rel_path, content) in text_files.iter() {
        let full_path = base_path.join(rel_path);
        if let Some(parent) = full_path.parent() {
            fs::create_dir_all(parent)?;
        }
        fs::write(&full_path, content)?;
        println!("[+] Сгенерирован файл оверлея: {:?}", rel_path);
    }

    // 2. РАЗВЕРТЫВАНИЕ DISPATCH-CONF В СИСТЕМУ
    println!("\n--- Синхронизация dispatch-conf... ---");
    let dispatch_src = base_path.join("sys-apps/pkgrs/files/dispatch-conf.fish");
    let dispatch_content = fs::read_to_string(&dispatch_src)?;
    let dispatch_targets = ["/usr/sbin/dispatch-conf", "/usr/lib/python-exec/dispatch-conf"];
    
    for target in &dispatch_targets {
        let path = Path::new(target);
        let _ = fs::remove_file(path);
        fs::write(path, &dispatch_content)?;
        fs::set_permissions(path, fs::Permissions::from_mode(0o755))?;
        println!("[🛡️] Барьер dispatch-conf выставлен: {}", target);
    }

    // 3. РАЗВЕРТЫВАНИЕ ETC-UPDATE В СИСТЕМУ
    println!("\n--- Синхронизация etc-update... ---");
    let etc_src = base_path.join("sys-apps/pkgrs/files/etc-update.fish");
    let etc_content = fs::read_to_string(&etc_src)?;
    
    // Добавляем пути, где система и сам Portage ищут etc-update
    let etc_targets = [
        "/usr/sbin/etc-update", 
        "/usr/lib/python-exec/etc-update",
        "/etc/portage/bin/etc-update", // Наш локальный путь репозитория/системы
    ];
    
    for target in &etc_targets {
        let path = Path::new(target);
        
        // Создаем родительскую папку /etc/portage/bin/, если её вдруг нет
        if let Some(parent) = path.parent() {
            let _ = fs::create_dir_all(parent);
        }
        
        let _ = fs::remove_file(path);
        fs::write(path, &etc_content)?;
        fs::set_permissions(path, fs::Permissions::from_mode(0o755))?;
        println!("[🛡️] Барьер etc-update выставлен: {}", target);
    }

    println!("\n\x1B[1;32m[SUCCESS]\x1B[0m Двойной барьер (dispatch-conf + etc-update) переведен под полный контроль Fish!");
    Ok(())
} "##),
...
"#, name = name, version = version, lib_name = lib_name, pc_requires = pc_requires, rdepend_raw = rdepend_raw, sources_dir = sources_dir)?;
    ...
    Ok(())
     ...
     ("sys-apps/pkgrs/files/src/bin/pkgrs-use-cache.rs", r#"// src/bin/pkgrs-use_cache.rs
use rusqlite::{params, Connection};
use std::fs;
use std::path::{Path};

const DB_PATH: &str = "/var/cache/portage/rust_index.db";
// ПЕРВОИСТОЧНИК ВНУТРИ ВАШЕГО GIT-ОВЕРЛЕЯ:
const PROFILE_USE_PATH: &str = "/var/db/repos/tupoll-overlay/profiles/targets/amd64/wayland/package.use";

fn main() -> Result<(), Box<dyn std::error::Error>> {
    println!(" \x1B[1;34m[>] \x1B[0m Компиляция матрицы USE-флагов из Git-профиля...");

    let base_path = Path::new(PROFILE_USE_PATH);
    if !base_path.exists() {
        println!(" \x1B[1;33m[!]\x1B[0m Профиль оверлея {} не найден. Пропуск.", PROFILE_USE_PATH);
        return Ok(());
    }
        ...
        if let Ok(entries) = fs::read_dir(base_path) {
            for entry in entries.flatten() {
                ...
                if p.is_file() {
                    ...
                    // Игнорируем скрытый мусор текстовых редакторов
                    if !name.starts_with('.') && !name.ends_with('~') && !name.ends_with(".bak") {
                        files_to_parse.push(p);
                    }
                }
            }
        }
    } else {
        files_to_parse.push(base_path.to_path_buf());
    }

        // Открываем базу данных с защитой от блокировок
    let mut conn = Connection::open(DB_PATH)?;

    // Настройка таймаута: ждем 5 секунд, если база занята сторожем
    conn.busy_timeout(std::time::Duration::from_secs(5))?;
    
    // Включаем WAL-режим для параллельной работы процессов
    let _ = conn.pragma_update(None, "journal_mode", "WAL");



    // 1. Создаем таблицу кэша USE-флагов
    conn.execute(
        "CREATE TABLE IF NOT EXISTS use_cache (
    ...
    // 2. Открываем быструю транзакцию
    let tx = conn.transaction()?;
    tx.execute("DELETE FROM use_cache", [])?;

    // 3. Линейно парсим файлы флагов из Git-оверлея
        ...
        if let Ok(content) = fs::read_to_string(&file_path) {
            for line in content.lines() {
                ...
                if let Some(package) = parts.next() {
                    ...
                    tx.execute(
                        "INSERT INTO use_cache (package, flags) 
                         VALUES (?1, ?2) 
                         ON CONFLICT(package) DO UPDATE SET flags = excluded.flags",
                        params![package, flags_str],
                    )?;
                }
            }
        }
    ...
    println!(" \x1B[1;32m[USE OK]\x1B[0m Матрица профиля оверлея успешно вшита в базу.");
    Ok(())
}  "#),
     ("sys-apps/pkgrs/files/src/bin/pkgrs.rs", r#"use nix::libc;
...
fn get_ebuild_iuse(ebuild_path: &Path) -> Vec<String> {
    let mut iuse_flags = Vec::new();
    if let Ok(file) = File::open(ebuild_path) {
        let reader = BufReader::new(file);
        let mut in_iuse = false;
        let mut iuse_accumulator = String::new();

        for line in reader.lines().flatten() {
            let trimmed = line.trim();
            
            if trimmed.starts_with("IUSE=") {
                in_iuse = true;
                iuse_accumulator.push_str(trimmed.trim_start_matches("IUSE="));
                // Если строка заканчивается на перенос или кавычка не закрыта, идем дальше
            ...
            if in_iuse {
                iuse_accumulator.push_str(" ");
                iuse_accumulator.push_str(trimmed.trim_end_matches('\\'));
                if !trimmed.ends_with('\\') {
                    break; // Строка IUSE закончилась
                }
            }
        }

        // Очищаем накопленную строку от мусора, кавычек и переменных
        let clean_str = iuse_accumulator.replace('"', "").replace('\'', "");
        for flag in clean_str.split_whitespace() {
            // Убираем дефолтные префиксы портежей вроде "+" или "-" (например, +asm -> asm)
            let clean_flag = flag.trim_start_matches('+').trim_start_matches('-').to_string();
            // Игнорируем наследование переменных вроде ${IUSE}
            if !clean_flag.is_empty() && !clean_flag.starts_with('$') {
                iuse_flags.push(clean_flag);
            }
        }
    }
    iuse_flags
   ...
   fn get_system_use_flags() -> HashSet<String> {
    ...
    if let Ok(out) = output {
        let use_str = String::from_utf8_lossy(&out.stdout);
        for flag in use_str.split_whitespace() {
            active_flags.insert(flag.to_string());
        }
    }
    active_flags
}

// Формируем красивую цветную строку в эталонном стиле Gentoo
fn format_ebuild_use_flags(ebuild_path: &Path) -> String {
    let iuse = get_ebuild_iuse(ebuild_path);
    if iuse.is_empty() {
        return "".to_string();
    }

    let system_use = get_system_use_flags();
    let mut formatted_flags = Vec::new();

    for flag in iuse {
        if system_use.contains(&flag) {
            // Включенные флаги подсвечиваем ярким красным/зеленым (как у вас в терминале)
            formatted_flags.push(flag.red().bold().to_string());
        } else {
            // Выключенные — синим цветом со знаком минус
            formatted_flags.push(format!("-{}", flag).blue().to_string());
        }
    }

    format!("USE=\"{}\"", formatted_flags.join(" "))
}

fn find_highest_ebuild(atom: &str, repo_base: &str) -> Result<PathBuf, String> {
    let repo_path = PathBuf::from(repo_base);
    let mut target_dir = PathBuf::new();

    if atom.contains('/') {
        let full_path = repo_path.join(atom);
        if full_path.is_dir() {
        ...
        if let Ok(entries) = fs::read_dir(&repo_path) {
            for entry in entries.flatten() {
                ...
                if category_path.is_dir() {
                    let potential_path = category_path.join(atom);
    ...
    if target_dir.as_os_str().is_empty() {
        return Err(format!("Пакет '{}' не найден в дереве Portage", atom));
    }
    ...
    if let Ok(entries) = fs::read_dir(target_dir) {
        for entry in entries.flatten() {
            let path = entry.path();
            if path.extension().map_or(false, |ext| ext == "ebuild") {
                ebuilds.push(path);
            }
        }
    }
    ...
    if ebuilds.len() > 1 {
        if let Some(highest) = ebuilds.last() {
            if highest.to_string_lossy().contains("9999") {
                return Ok(ebuilds[ebuilds.len() - 2].clone());
            }
        }
    }
    ...
    if let Some(highest) = ebuilds.last() {
        ...
        Err(format!("В директории пакета '{}' нет .ebuild файлов", atom))
    }
}

// Нативная передача управления в python-exec-2 без лишних аргументов и сообщений на экране
fn pass_to_original_portage() -> Result<i32, PortageError> {
    use std::ffi::CString;
    use std::os::unix::ffi::OsStrExt;

    // Выставляем внутренний системный маркер Portage
    //env::set_var("PORTAGE_INTERNAL_CALLER", "1");

    // Жесткий системный путь к обертке, где прописан ваш Rust-ускоритель поиска
    let python_exec_path = "/usr/lib/python-exec/python-exec-2";
    let path_c = CString::new(python_exec_path).unwrap();
    
    // Подменяем argv строкой "emerge", чтобы ваш оригинальный python-exec-rs
    // обошел встроенную защиту и применил инжектор-ускоритель поиска
    let mut args_c: Vec<CString> = vec![
        CString::new("emerge").unwrap()
    ];
    
    // Пробрасываем все оригинальные системные флаги наружу в исходном виде
    for arg in env::args_os().skip(1) {
        args_c.push(CString::new(arg.as_bytes()).unwrap());
    }
    
    let c_args: Vec<*const libc::c_char> = args_c.iter()
        .map(|arg| arg.as_ptr())
        .chain(std::iter::once(std::ptr::null()))
        .collect();
        
    unsafe {
        // Моментальное замещение памяти без посредников, петель и ступора!
        libc::execv(path_c.as_ptr(), c_args.as_ptr());
        libc::perror(CString::new("execv failed to python-exec-2").unwrap().as_ptr());
    }

    eprintln!("Ошибка: Не удалось запустить /usr/lib/python-exec/python-exec-2");
    Ok(1)
}

fn emerge_main(args: &[String]) -> Result<i32, PortageError> {
    if args.len() < 2 {
        println!("{}", EMERGE_USAGE);
        return Ok(0);
    }
    ...
    if args.contains(&"--version".to_string()) || args.contains(&"-V".to_string()) {
        println!("Portage 3.0.65-rust (pkgrs frontend replacement)");
        return Ok(0);
    }

   
       // --- ПЕРЕХВАТ СИСТЕМНЫХ КОМАНД И СЕТОВ ОБНОВЛЕНИЯ ЧЕРЕЗ ПРЯМОЙ EXECV ---
    let has_world = args.iter().any(|arg| arg == "world" || arg == "@world" || arg == "system" || arg == "@system");
    let has_search = args.contains(&"-s".to_string()) || args.contains(&"--search".to_string());
    let has_sync = args.contains(&"--sync".to_string()) || args.contains(&"--depclean".to_string());

    // ИСПРАВЛЕНО: Никаких lmerge! Прямой наглый execv в python-exec-2
    if has_world || has_search || has_sync {
        return pass_to_original_portage(); // Моментальный уход в оригинальный Portage БЕЗ лишних строк на экране!
    }

    


    // --- УМНЫЙ И АККУРАТНЫЙ ПАРСИНГ ЛОКАЛЬНЫХ АРГУМЕНТОВ ---
    let mut options = Vec::new();
    let mut packages = Vec::new();

    let mut is_verbose = false;
    let mut is_ask = false;
    let mut is_pretend = false;
    let mut is_unmerge = false;
    ...
    for arg in &args[1..] {
        if arg.starts_with("--") {
            ...
            match arg.as_str() {
                ...
                "--ask" => is_ask = true,
                "--pretend" => is_pretend = true,
                "--unmerge" => is_unmerge = true,
                "--oneshot" => is_oneshot = true,
                _ => {}
            }
        } else if arg.starts_with('-') && arg.len() > 1 {
            options.push(arg);
            // Посимвольно разбираем склеенные короткие флаги (-av, -pv, -ov и т.д.)
    ...
    if packages.is_empty() {
        let needs_packages = is_oneshot || is_ask || is_pretend || is_verbose || is_unmerge;
        if needs_packages {
            eprintln!("emerge: No packages specified.");
            return Ok(1);
        }
    }

    if !packages.is_empty() {
        let repo_base_path = "/var/db/repos/gentoo";
        let mut jobs = Vec::new();

        for package in &packages {
            match find_highest_ebuild(package, repo_base_path) {
                Ok(path) => jobs.push((package, path)),
                Err(e) => return Err(PortageError::PackageNotFound(e)),
            }
        }

        // --- УДАЛЕНИЕ ОДИНОЧНЫХ ПАКЕТОВ (ЧЕРЕЗ QMERGE) ---
        if is_unmerge {
            if is_verbose || is_pretend || is_ask {
                println!("\nThese are the packages that would be unmerged:");
                for (pkg, _) in &jobs {
                    println!("  [{}] {}", "unmerge".truecolor(255, 128, 171).bold(), pkg);
                }
            }
            if is_pretend { return Ok(0); }
            if is_ask {
            ...
            for (pkg, _) in jobs {
                println!("{}", format!(">>> Удаление пакета {} из системы...", pkg).truecolor(255, 128, 171));
                let _ = Command::new("/usr/bin/qmerge").arg("-U").arg("-y").arg(pkg).status();
            }
            return Ok(0);
        }
                // --- НАШ СОБСТВЕННЫЙ НАТИВНЫЙ ВЫВОД (РЕАКЦИЯ НА ФЛАГИ -pv И -av) ---
        if is_verbose || is_pretend || is_ask {
            println!("\nThese are the packages that would be merged, in order:\n");
            
            for (pkg, ebuild_path) in &jobs {
                // Извлекаем IUSE из файла и сопоставляем с базой portageq
                let use_string = format_ebuild_use_flags(ebuild_path);
                
                // Печатаем строку — точь-в-точь как оригинальный emerge, но за миллисекунды!
                println!(
                    "[ebuild   R ] {}::gentoo  {}",
                    pkg.cyan(), 
                    use_string
        ...
        // Если был флаг pretend (-pv), завершаем работу — мы всё красиво показали
        if is_pretend { 
            return Ok(0); 
        }
        
        // Если был флаг ask (-av), запрашиваем подтверждение у пользователя
        if is_ask {
            print!("Would you like to merge these packages? [Yes/No] ");
            let _ = io::stdout().flush();
            let mut input = String::new();
            if io::stdin().read_line(&mut input).is_ok() {
                let trimmed = input.trim().to_lowercase();
        ...
        // --- УСТАНОВКА ОДИНОЧНЫХ ПАКЕТОВ (ЧЕРЕЗ ВАШ СВЕРХБЫСТРЫЙ ДВИЖОК EBUILD) ---
        if is_verbose || is_pretend || is_ask {
            println!("\nThese are the packages that would be merged, in order:");
            for (pkg, path) in &jobs {
                println!(
                    "  [{}] {}::gentoo [из файла: {}] USE=\"\"",
                    "ebuild   R    ".truecolor(255, 128, 171).bold(),
                    pkg,
        ...
        for (pkg, ebuild_path) in jobs {
println!("{}", format!(">>> Начинается сборка пакета {} через системный движок ebuild...", pkg).truecolor(255, 128, 171));
let phases = ["fetch", "unpack", "prepare", "configure", "compile", "install", "qmerge"];
for phase in &phases {
if is_verbose {
println!("{}", format!(">>> Исполнение фазы: {}", phase).truecolor(255, 128, 171));
}
let exit_status = Command::new("/usr/bin/ebuild")
.arg(&ebuild_path)
.arg(phase)
.status();
match exit_status {
...
Ok(status) => {
eprintln!("Ошибка: Фаза '{}' пакета '{}' завершилась неуспешно: {}", phase, pkg, status);
return Ok(status.code().unwrap_or(1));
}
Err(e) => {
eprintln!("Не удалось запустить /usr/bin/ebuild: {}", e);
return Ok(1);
}
}
}
println!("{}", format!(">>> Установка пакета {} успешно завершена.", pkg).truecolor(255, 128, 171));
}
}
Ok(0)
}
fn finalize_mod_echo() {
let _ = io::stdout().flush();
...
fn main() {
    ...
    // 1. МГНОВЕННЫЙ ВЫХОД ЕСЛИ ПУСТО
    if args.is_empty() {
        std::process::exit(0);
    }

    // 2. ЖЕСТКИЙ ПЕРЕХВАТ КУЗНИЦЫ (Вливаем из Файла 1)
    // Если первый аргумент — наш кастомный генератор, перехватываем ДО системного фильтра!
    let first_arg = &args[0];
    
         // 3. Поиск -s (комбинированный и неубиваемый)
    if args[0] == "-s" || args[0] == "--search" {
        let query = args.get(1).expect("Укажите запрос");
        
        println!("{} {}", ">>>".green(), "Запуск комбинированного поиска Pinnacle OS...".bold());

        // ЧАСТЬ 1: Поиск в системном Portage. 
        // Вместо слепого Command::new("emerge") бьем точно в python-exec-2 с подменой имени процесса на "emerge"!
        // Это задействует ваш инжектор-ускоритель в заголовках Python, не вызывая ступора и петель.
        use std::os::unix::process::CommandExt;
        let mut sys_cmd = Command::new("/usr/lib/python-exec/python-exec-2");
        sys_cmd.arg0("emerge");
        sys_cmd.arg("-s");
        sys_cmd.arg(query);
        
        // Честно дожидаемся вывода системных результатов
        let _ = sys_cmd.status();

        // ЧАСТЬ 2: Поиск в вашей локальной базе Cargo-base (crates.io)
        // Вызывается сразу вслед за системным, без запинок
        let _ = Command::new("emerge-cargo-base")
            .arg(query)
            .status();
    ...
    if first_arg == "-sc" || first_arg == "--search-cargo" {
        let status = Command::new("emerge-cargo-base").args(&args[1..]).status().unwrap_or_else(|_| std::process::exit(1));
        std::process::exit(status.code().unwrap_or(0));
    }
    if first_arg == "-wl" || first_arg == "--writer-lib" {
        let status = Command::new("writer-lib").args(&args[1..]).status().unwrap_or_else(|_| std::process::exit(1));
        std::process::exit(status.code().unwrap_or(0));
    }
    if first_arg == "-wle" || first_arg == "--emerge-setup-lib" {
        let status = Command::new("emerge-setup-lib").args(&args[1..]).status().unwrap_or_else(|_| std::process::exit(1));
        std::process::exit(status.code().unwrap_or(0));
    }
    if first_arg == "-wb" || first_arg == "--writer-bin" {
        let status = Command::new("writer-bin").args(&args[1..]).status().unwrap_or_else(|_| std::process::exit(1));
        std::process::exit(status.code().unwrap_or(0));
    }
    if first_arg == "-wbe" || first_arg == "--emerge-setup-bin" {
        let status = Command::new("emerge-setup-bin").args(&args[1..]).status().unwrap_or_else(|_| std::process::exit(1));
        std::process::exit(status.code().unwrap_or(0));
    }
    if first_arg == "-wg" || first_arg == "--writer-git" {
        let status = Command::new("writer-git").args(&args[1..]).status().unwrap_or_else(|_| std::process::exit(1));
        std::process::exit(status.code().unwrap_or(0));
    }
    if first_arg == "-wge" || first_arg == "--emerge-setup-git" {
        let status = Command::new("emerge-setup-git").args(&args[1..]).status().unwrap_or_else(|_| std::process::exit(1));
        std::process::exit(status.code().unwrap_or(0));
    }
    
    if first_arg == "-wgw" || first_arg == "--writer-git-windows" {
        let status = Command::new("writer-git-windows").args(&args[1..]).status().unwrap_or_else(|_| std::process::exit(1));
        std::process::exit(status.code().unwrap_or(0));
    }
    if first_arg == "-wpa" || first_arg == "--writer-pc-all" {
        let status = Command::new("writer-pc-all").args(&args[1..]).status().unwrap_or_else(|_| std::process::exit(1));
        std::process::exit(status.code().unwrap_or(0));
    }
    
    if first_arg == "-wpl" || first_arg == "--writer-pc-world" {
        let status = Command::new("writer-pc-world").args(&args[1..]).status().unwrap_or_else(|_| std::process::exit(1));
        std::process::exit(status.code().unwrap_or(0));
    }
    if first_arg == "-u" || first_arg == "--emerge-update" {
        let status = Command::new("emerge-update").args(&args[1..]).status().unwrap_or_else(|_| std::process::exit(1));
        std::process::exit(status.code().unwrap_or(0));
    }
    if first_arg == "-wf" || first_arg == "--writer-font" {
        let status = Command::new("writer-font").args(&args[1..]).status().unwrap_or_else(|_| std::process::exit(1));
        std::process::exit(status.code().unwrap_or(0));
    }
    
    if first_arg == "-ws" || first_arg == "--writer-ws" {
        let status = Command::new("writer-ws").args(&args[1..]).status().unwrap_or_else(|_| std::process::exit(1));
        std::process::exit(status.code().unwrap_or(0));
    }
    
    if first_arg == "-m" || first_arg == "--man" {
        let status = Command::new("pkgrs-man").args(&args[1..]).status().unwrap_or_else(|_| std::process::exit(1));
        std::process::exit(status.code().unwrap_or(0));
    }
    
    
    if first_arg == "-l" || first_arg == "--lmerge" {
        let status = Command::new("lmerge").args(&args[1..]).status().unwrap_or_else(|_| std::process::exit(1));
        std::process::exit(status.code().unwrap_or(0));
    }

    // =========================================================================
    // 3. ТОЛЬКО ТЕПЕРЬ ВКЛЮЧАЕТСЯ НАШ СИСТЕМНЫЙ ФИЛЬТР ЧЕРЕЗ ДЕРЕВО EBUILD
    // (Все, что не перехвачено кузницей выше — улетает в честный анализ)
    // =========================================================================
    let mut _belongs_to_rust = false;
...
match emerge_main(&args) {
Ok(retval) => process::exit(retval),
Err(_) => process::exit(1),
}
}
extern "C" fn handle_sigterm(_signum: libc::c_int) { process::exit(143); }
extern "C" fn handle_sigint(_signum: libc::c_int) { finalize_mod_echo(); process::exit(130); }
 "#),
     ("sys-apps/pkgrs/files/src/bin/pkgrs-man.rs", r#"use std::{thread, io::{self, Read, Write}, time::Duration};
use std::process::Command;
use colored::Colorize;
// ==========================================
...
fn my_custom_logic() {
	println!("{}", "PKGRS — СИСТЕМА УПРАВЛЕНИЯ ПАКЕТАМИ (RUST + GENTOO)".bold().green());
        ...
        println!("  lmerge — это лёгкий emerge-полноценный помошник,может все функции портаж");
        println!("  системные репозитории с экосистемой Cargo. При поиске");
        println!("  выводятся данные из локальной SQLite БД (emerge-cargo-base).");
        println!("\n{}", "КОМАНДЫ:".yellow());
        println!(" pkgrs -l :"); println!("{}",  "-l --lmerge(любая команда: -av, --ask, -pv, -s ....".yellow());       
        ...
        println!("\n{}", "WORKSPACE:".yellow());
        println!(" C использованием .lib .bin нескольких файлов(sudo-rs):");println!("  {}", "-ws, --writer-ws: <pkg> <version>".green());
        println!("\n{}", "Шрифты:".yellow());
        println!(" Для извлечения шрифтов (.ttf) из крейтов:");
        println!("  {}", "-wf, --writer-font <pkg> <version> <constants>".green());
        println!("\n{}", "Пример:".yellow());
        println!("  {}", "pkgrs -wf aetna-fonts-roboto 0.1.0 ROBOTO_REGULAR,ROBOTO_BOLD".green());
...
fn main() {
    ...
    let _ = io::stdout().flush();
     ...
     ("sys-apps/pkgrs/files/src/bin/rust-openrc-helper.rs", r##"// src/bin/rust-openrc-helper.rs
...
fn main() -> std::io::Result<()> {
    ...
    // КРИСТАЛЬНО ЧИСТЫЙ КОД ВАТЧЕРА (Контроль через ваш dispatch-setup)
    let watcher_script = r#"#!/bin/sh
# =========================================================================
# SMART REACTIVE WATCHER FOR OPENRC (PURE POSIX SH MODE)
# OPTIMIZED RESOURCE MANAGEMENT — NO EXTRA DISK IO
# =========================================================================
WATCH_PATH="/var/db/repos"
LOG_FILE="/var/log/rust-portage-watcher.log"
SIG_FILE="/etc/pkgrs/signatures.hash"

exec > "$LOG_FILE" 2>&1

# Быстрая фаза проверки подписей (оставляем регулярной, диск не грузит)
verify_and_heal_barriers() {
    [ ! -f "$SIG_FILE" ] && return 0
    if ! sha256sum --status --check "$SIG_FILE"; then
        echo "[🛡️ WATCHER ALERT] Обнаружена подмена барьеров! Автолечение..."
        pkgrs-dispatch-setup
    fi
}
...
# Первичный прогон при старте системы
verify_and_heal_barriers
rust-helper sync
pkgrs-use-cache

echo "[>] Сторож успешно переведен в режим умного энергосбережения пулов."

# Основной реактивный цикл ожидания
# Слушаем изменения только в конкретных критических узлах оверлея!
inotifywait -q -m -r -e close_write,move,create,delete "$WATCH_PATH" | while read -r directory event file; do
    
    # 1. Защиту подписей проверяем всегда и мгновенно
    verify_and_heal_barriers
    
    # 2. ФИЛЬТР ЗАРАЗЫ И ОВЕРХЕДА:
    # Тяжелый пересчет базы и флагов запускаем ТОЛЬКО если изменения произошли 
    # в файлах профиля (package.use) или прилетели свежие ебилды.
    # Полностью игнорируем мусорные файлы блокировок и временные директории сборки!
    if echo "$directory" | grep -qE "(profiles|metadata|package.use|\.ebuild)"; then
        
        # Умный троттлинг: ждем 10 секунд, пока завершится массовая операция (например, git pull)
        sleep 10
        
        echo "[🛡️ WATCHER] Зафиксировано системное изменение в оверлее. Синхронизация..."
        rust-helper sync
        pkgrs-use-cache
        echo "[🛡️ WATCHER] База данных и матрица USE-флагов успешно обновлены."
    fi
done

"#;
     
     let openrc_service = r#"#!/sbin/openrc-run
description="Monitors Portage repos and updates Rust index with Zero-Trust protection"
command="/usr/local/bin/rust-portage-watcher.sh"
command_background=true
pidfile="/run/${RC_SVCNAME}.pid"
...
"#;
    ...
    println!("--- Настройка Нативного OpenRC Помощника pkgrs ---");

    // 1. Записываем скрипт ватчера в систему
    fs::write("/usr/local/bin/rust-portage-watcher.sh", watcher_script)?;
    fs::set_permissions("/usr/local/bin/rust-portage-watcher.sh", fs::Permissions::from_mode(0o755))?;
    
    // 2. Записываем службу OpenRC
    fs::write("/etc/init.d/rust-portage-watcher", openrc_service)?;
    fs::set_permissions("/etc/init.d/rust-portage-watcher", fs::Permissions::from_mode(0o755))?;

    // 3. Запускаем ваш dispatch-setup прямо отсюда, чтобы он сразу создал первый signatures.hash
    println!("[>] Запуск pkgrs-dispatch-setup для первичной разметки...");
    let _ = Command::new("pkgrs-dispatch-setup").status();

    // Безопасное обновление sysctl.conf
    let limit_line = "fs.inotify.max_user_watches=524288";
    ...
    Command::new("sysctl").arg("-w").arg(limit_line).status().ok();
    println!("[+] Конфиги и лимиты обновлены.");

    // Очистка старых хвостов
    println!("[>] Перезапуск службы сторожа в OpenRC...");
    Command::new("rc-service").arg("rust-portage-watcher").arg("stop").status().ok();
    Command::new("pkill").arg("-f").arg("rust-portage-watcher.sh").status().ok();
    thread::sleep(Duration::from_millis(800));

    // Запуск службы
    Command::new("rc-update").arg("add").arg("rust-portage-watcher").arg("default").status().ok();
    let output = Command::new("rc-service")
        .arg("rust-portage-watcher")
        .arg("start")
        .output()?;

    if output.status.success() {
        println!("\n\x1B[1;32m[GLOBAL OK]\x1B[0m Служба OpenRC запущена. Контроль подписей замкнут на pkgrs-dispatch-setup!");
    } else {
        let _ = Command::new("/etc/init.d/rust-portage-watcher").arg("start").status();
    }

    Ok(())
} "##),
   ("sys-apps/pkgrs/files/src/bin/lmerge.rs", r#"// Copyright 2006-2026 Gentoo Authors
// Distributed under the terms of the GNU General Public License v2
...
fn main() {
    ...
    // Путь к исполнителю скриптов оставляем прежним
    let path = CString::new("/usr/lib/python-exec/python-exec-2").unwrap();
    
    // ВАЖНО: Первым элементом (argv[0]) передаем "emerge".
    // Именно по этому имени python-exec поймет, какой скрипт из /usr/lib/python-exec/ в реальности дергать.
    let mut args: Vec<CString> = vec![
        CString::new("emerge").unwrap(),
    ];
    
    // Пробрасываем все остальные аргументы командной строки
    for arg in env::args_os().skip(1) {
  ...
  ("sys-apps/pkgrs/files/src/bin/font.rs", r##"use std::fs;
use std::process::Command;
use std::path::Path;
use colored::*;
use std::io::Write;
...
fn main() -> std::io::Result<()> {
    let args: Vec<String> = std::env::args().collect();
    // Принимаем имя, версию и константы (например: REGULAR,BOLD,ITALIC)
    if args.len() < 4 {
        println!("{}", "Использование:".bold().yellow());
        println!("  pkgrs writer-font <pkg> <version> <constants_comma_separated>");
        println!("\n{}", "Примеры:".bold().green());
        println!("  pkgrs writer-font aetna-fonts-roboto 0.1.0 REGULAR,MEDIUM,BOLD,ITALIC");
        return Ok(());
    }

    let name = &args[1];
    let version = &args[2];
    let constants_raw = &args[3];
    let crate_mod = name.replace("-", "_");

    // Разбиваем строку констант на вектор
    let constants: Vec<&str> = constants_raw.split(',').map(|s| s.trim()).collect();

    println!("{} {} {}", ">>>".green(), "Инициализация экстрактора шрифтов для".bold(), name.cyan());
    ...
    // 1. Создаем бинарное приложение-дампер
    Command::new("cargo")
        .args(["new", "--bin", "build-root"])
        .current_dir(&sources_dir)
        .status()?;

    // 2. Добавляем целевой крейт в зависимости
    Command::new("cargo")
        .args(["add", &format!("{}@{}", name, version)])
        .current_dir(&work_dir)
        .status()?;

    // 3. Вендорим код локально
    Command::new("cargo")
        .args(["vendor", "../vendor"])
        .current_dir(&work_dir)
        .status()?;

    // 4. ГЕНЕРИРУЕМ src/main.rs (генератор файлов из байт)
    let mut main_rs_body = String::new();
    for constant in &constants {
        // Имя файла на выходе, например: aetna-fonts-roboto-REGULAR.ttf
        let file_name = format!("{}-{}.ttf", name, constant.to_lowercase());
        main_rs_body.push_str(&format!(
            "    {{\n\
                     let bytes = {0}::{1};\n\
                     let mut f = File::create(\"{2}\").unwrap();\n\
                     f.write_all(&bytes).unwrap();\n\
                     println!(\"   [+] Выгружен файл: {2}\");\n\
                 }}\n",
            crate_mod, constant, file_name
        ));
    }
    ...
    let main_rs_content = format!(
        "use std::fs::File;\n\
         use std::io::Write;\n\
         fn main() {{\n\
             println!(\"--- Извлечение шрифтов из бинарных массивов ---\");\n\
         {}\n\
         }}",
        main_rs_body
    );
    fs::write(format!("{}/src/main.rs", work_dir), main_rs_content)?;

    // 5. ГЕНЕРИРУЕМ EBUILD
    let ebuild_path = format!("{}/{}-{}.ebuild", pkg_dir, name, version);
    let mut f = fs::File::create(&ebuild_path)?;

    writeln!(f, "#writer-font")?;
    writeln!(f, "EAPI=8\ninherit cargo font\n\nDESCRIPTION=\"Fonts extracted from bytes of {}\"\nHOMEPAGE=\"https://crates.io{}\"\nSRC_URI=\"\"\nS=\"${{WORKDIR}}/build-root\"", name, name)?;
    writeln!(f, "LICENSE=\"MIT\"\nSLOT=\"0\"\nKEYWORDS=\"~amd64\"")?;
    writeln!(f, "FONT_SUFFIX=\"ttf\"")?;
...
directory = '../vendor'\" > .cargo/config.toml || die
    ...
    # Запуск дампера для генерации физических .ttf файлов перед установкой
    ./target/release/build-root || die
}}")?;

    writeln!(f, "\nsrc_install() {{
    insinto /usr/share/fonts/{}
    # Находим все созданные дампером ttf файлы в корне сборки
    find . -maxdepth 1 -name '*.ttf' -exec doins {{}} +
    font_src_install
}}", name)?;

    // Права и Manifest портежей
    let _ = Command::new("chown").args(["-R", "portage:portage", &pkg_dir]).status();
    let _ = Command::new("chmod").args(["-R", "go+rX", &pkg_dir]).status();
    Command::new("ebuild").arg(&ebuild_path).arg("digest").status()?;

    println!("{} Скрипт-экстрактор успешно добавлен! {}/{}", ">>>".green(), CATEGORY, name);
    Ok(())
}   
 "##),  
...
for line in reader.lines() {
    ...
    println!("  {} Запуск установки: pkgrs -l {}", ">>>".blue(), pkg_atom);
...
fn main() -> io::Result<()> {
    // 1. СОЗДАНИЕ ОРДЕНА (Юзер и Группа)
     let username = "pkgrs";
    let uid = "256"; // Безопасный UID/GID (Portage сидит на 250)
    let groups = "root,adm,disk,wheel,portage,users";

    println!("{} Настройка системной группы {} (GID {})...", ">>>".green(), username, uid);
    let group_status = Command::new("groupadd")
        .args(["-g", uid, "-r", username])
        .status()?;

    if !group_status.success() {
        println!("{} Группа {} уже существует или GID занят.", " [!]".yellow(), username);
    }

    println!("{} Настройка системного пользователя {} (UID {})...", ">>>".green(), username, uid);
    ...
    if user_status.success() {
        println!("{} Пользователь {} успешно создан с группами: {}", " [+]".green(), username, groups);
       } else {
        println!("{} Пользователь уже существует. Принудительное исправление UID/GID на 256 и синхронизация групп...", " [!]".yellow());
        
        // 1. Сдвигаем GID группы pkgrs на 256
        let _ = Command::new("groupmod").args(["-g", "256", username]).status();

        // 2. Сдвигаем UID пользователя на 256 и обновляем все группы
               let mod_status = Command::new("usermod")
            .args(["-u", "256", "-g", "256", "-G", groups, username])
            .status()?;
            
        if !mod_status.success() {
            return Err(std::io::Error::new(
                std::io::ErrorKind::Other, 
                "Не удалось принудительно обновить UID/GID для pkgrs"
            ));
        }
        
        // Эта строка должна быть СНАРУЖИ блока 'if !mod_status.success()', в самом конце 'else'
        println!("{} UID/GID успешно изменены на 256. Группы синхронизированы.", " [+]".green());
...
fn create_and_install_ebuild(name: &str) -> io::Result<()> {
    ...
    Command::new("pkgrs -l").arg("--ask").arg(format!("{}/{}", CATEGORY, clean_name)).status()?;
    Ok(())
}
...
fn main() -> std::io::Result<()> {
    ...
    let target_file = "/var/db/repos/tupoll-overlay/sys-apps/pkgrs/files/src/bin/python-exec-rs.rs";
    ...
    if let Some(index) = selected_index {
        if index > 0 && index <= pythons.len() {
            ...
            if found {
                ...
                let emerge_status = Command::new("pkgrs")
                    .arg("-l")
                    .arg("sys-apps/pkgrs")
                    .status()?;
...
fn generate_iuse_and_features(cargo_toml_path: &str) -> (String, String) {
    let mut use_map = HashMap::new();
    
    if let Ok(content) = fs::read_to_string("/etc/pkgrs/use_mappings.conf") {
        for line in content.lines() {
            if line.trim().is_empty() || line.starts_with('#') { continue; }
            let parts: Vec<&str> = line.split('=').map(|s| s.trim()).collect();
            if parts.len() == 2 {
                use_map.insert(parts[0].to_string(), parts[1].to_string());
            }
        }
    }

    let mut found_gentoo_flags = Vec::new();
    let mut bash_feature_logic = String::new();
    
    if let Ok(toml) = fs::read_to_string(cargo_toml_path) {
        let mut in_features_section = false;
        
        for line in toml.lines() {
            let trimmed = line.trim();
            if trimmed.starts_with('[') && !trimmed.starts_with("[features]") {
            ...
            if in_features_section {
                for (gentoo_flag, cargo_feature) in &use_map {
                    // ТОЧНАЯ ПРОВЕРКА: Ищем фичу строго как отдельное слово с пробелом или в кавычках
                    let pattern1 = format!("{} =", cargo_feature);
                    let pattern2 = format!("\"{}\"", cargo_feature);
                    
                    if trimmed.starts_with(&pattern1) || trimmed.contains(&pattern2) {
                        if !found_gentoo_flags.contains(gentoo_flag) {
                            found_gentoo_flags.push(gentoo_flag.clone());
                        }
                        // Пробрасываем в Cargo ТОЛЬКО если в имени нет знака '+', чтобы не злить Portage
                        if !cargo_feature.contains('+') {
                            bash_feature_logic.push_str(&format!("    use {} && myfeatures+=(\"{}\")\n", gentoo_flag, cargo_feature));
                        }
                    }
                }
            }
    ...
    let iuse_string = found_gentoo_flags.join(" ");
    (iuse_string, bash_feature_logic)
}
...
fn main() -> std::io::Result<()> {
    ...
    // Бронебойная обрезка версии от метаданных (+) для путей Portage
    let raw_version = &args[2];
    let version = raw_version.split('+').next().unwrap();
    ...
    // Передаем оригинальную raw_version для скачивания через Cargo
    let dl = Command::new("cargo")
        .args(["download", "-x", &format!("{}=={}", name, raw_version)])
        .current_dir(&sources_dir)
        .status();
    ...
    if dl.is_err() || !dl.unwrap().success() {
        ...
        let _ = fs::remove_dir_all(&tmp_path);
        Command::new("cargo").args(["new", "tmp_dl"]).current_dir(&sources_dir).output()?;
        Command::new("cargo").args(["add", &format!("{}@{}", name, raw_version)]).current_dir(&tmp_path).output()?;
        Command::new("cargo").arg("vendor").current_dir(&tmp_path).output()?;
    ...
    // 1. Запускаем динамический расчет IUSE по Cargo.toml
    let cargo_toml_path = format!("{}/Cargo.toml", work_dir);
    let (_iuse_str, mut bash_logic) = generate_iuse_and_features(&cargo_toml_path);

    // 2. ДОП ФУНКЦИОНАЛ: Читаем принудительные фичи из внешнего файла пакета (например, /etc/pkgrs/env/ironbar.features)
    let custom_features_path = format!("/etc/pkgrs/env/{}.features", name);
    if Path::new(&custom_features_path).exists() {
        if let Ok(forced_content) = fs::read_to_string(&custom_features_path) {
            println!("{} Обнаружены принудительные фичи для пакета {}", ">>>".cyan(), name);
            for raw_feature in forced_content.lines() {
                let feature = raw_feature.trim();
                if feature.is_empty() || feature.starts_with('#') { continue; }
                
                // В наглую внедряем фичу в bash_logic компиляции, минуя парсер
                bash_logic.push_str(&format!("    myfeatures+=(\"{}\")\n", feature));
            }
        }
    }

    let _rdepend = get_system_libs(&work_dir);
    let ebuild_path = format!("{}/{}-{}.ebuild", pkg_dir, name, version);
    let _f = fs::File::create(&ebuild_path)?;
    
       // 1. Запускаем динамический расчет IUSE по Cargo.toml
    let cargo_toml_path = format!("{}/Cargo.toml", work_dir);
    let (iuse_str, mut bash_logic) = generate_iuse_and_features(&cargo_toml_path);
    ...
    // ... (запись шапки ebuild, LICENSE, KEYWORDS, IUSE остаются прежними) ...
    writeln!(f, "#writer-bin")?;
    writeln!(f, "EAPI=8\ninherit cargo\n\nDESCRIPTION=\"{}\"\nHOMEPAGE=\"https://crates.io{}\"\nSRC_URI=\"\"\nS=\"${{WORKDIR}}/{}-{}\"", name, name, name, version)?;
    writeln!(f, "LICENSE=\"MIT\"\nSLOT=\"0\"\nKEYWORDS=\"~amd64\"")?;
    writeln!(f, "IUSE=\"{}\"", iuse_str)?;
    writeln!(f, "RDEPEND=\"\n\t{}\"\nDEPEND=\"${{RDEPEND}}\"\nBDEPEND=\"virtual/pkgconfig\"", rdepend)?;

    writeln!(f, "\nsrc_unpack() {{ cp -Rp \"{}/\"* \"${{WORKDIR}}/\" || die; }}", sources_dir)?;
    
          // Бронебойный src_compile с защитой от скрытого оверрайда eclass Gentoo
    writeln!(f, "\nsrc_compile() {{
    export CARGO_HOME=\"${{T}}/cargo_home\"
    mkdir -p .cargo || die
...
directory = 'vendor'\" > .cargo/config.toml || die
    ...
    # КРИТИЧЕСКИЙ МОМЕНТ: Сбрасываем автоматический проброс фич от eclass Gentoo,
    # который насильно пихал ложные 'gtk4' и 'wayland' в компилятор
    unset CARGO_FEATURE_FLAGS

    # Динамический сборщик фич Cargo на основе НАШЕЙ ручной матрицы
    local myfeatures=()

{}{}
    local feature_list=$(IFS=,; echo \"${{myfeatures[*]}}\")

    if [[ -z \"${{RUSTFLAGS}}\" ]]; then
        export RUSTFLAGS=\"-C target-cpu=native\"
    fi

    # Сборка в стандартном дефолтном режиме, но строго с нашими переключаемыми фичами
    if [[ -n \"${{feature_list}}\" ]]; then
        cargo build --release --offline --features \"${{feature_list}}\" || die
    else
        cargo build --release --offline || die
    fi
}}", bash_logic, "    # Конец dynamic блока\n")?;


    // Универсальный src_install с поддержкой ВНЕШНЕЙ конфигурации (.env)
    writeln!(f, "\nsrc_install() {{
    find target/release -maxdepth 1 -executable -type f ! -name \"*.so\" -exec dobin {{}} +")?;

    let custom_env_path = format!("/etc/pkgrs/env/{}.env", name);
    if Path::new(&custom_env_path).exists() {
        if let Ok(custom_instructions) = fs::read_to_string(&custom_env_path) {
            writeln!(f, "\n    # --- Внешняя конфигурация pkgrs ---")?;
            writeln!(f, "    {}", custom_instructions.trim())?;
        }
    }
    writeln!(f, "}}")?;
...
fn main() -> std::io::Result<()> {
    ...
    // --- ДОБАВЛЕНО ТОЛЬКО ЭТО УСЛОВИЕ ---
    // Берем аргумент версии и, если есть '+', отсекаем его.
    let version = args[2].split('+').next().unwrap();
    // -------------------------------------
    ...
    println!("{} Инициализация проекта-обертки...", ">>>".green());
    // Дальше идет ваш оригинальный код без изменений...
...
directory = '../vendor'\" > .cargo/config.toml || die
 ...
 "##),
    ("sys-apps/pkgrs/files/src/bin/writer-ws.rs", r##"use std::fs;
use std::process::Command;
use std::io::Write;
use colored::*;
use std::collections::HashSet;
