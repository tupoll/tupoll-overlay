...
"#, name = name, version = version, lib_name = lib_name, pc_requires = pc_requires, rdepend_raw = rdepend_raw, sources_dir = sources_dir)?;
    ...
    Ok(())
     ...
     ("sys-apps/pkgrs/files/src/bin/rust-openrc-helper.rs", r##"use std::fs::{self, OpenOptions};
use std::io::Write;
...
"#;
    ...
    println!("--- Настройка Rust Portage Accelerator ---");
    ...
    println!("[>] Очистка старых процессов...");
    ...
    // Запуск
    println!("[>] Добавление в автозагрузку и запуск...");
    ...
    if output.status.success() {
        println!("[!] СЕРВИС ЗАПУЩЕН УСПЕШНО!");
    } else {
        println!("[?] Стандартный запуск не удался, пробуем прямой вызов...");
        let fallback = Command::new("/etc/init.d/rust-portage-watcher").arg("start").status()?;
        if fallback.success() { println!("[!] Запущено через прямой вызов!"); }
    }
...
fn main() {
  ...
  ("sys-apps/pkgrs/files/src/bin/font.rs", r##"   
...
("sys-apps/pkgrs/files/src/bin/emerge.rs", r#"use nix::libc;
...
// Вспомогательная функция для бесшовной передачи власти в lmerge без дублирования ОЗУ
fn pass_to_lmerge(args: &[String], pink_msg: &str) -> Result<i32, PortageError> {
    use std::ffi::CString;
    use std::os::unix::ffi::OsStrExt;

    if !pink_msg.is_empty() {
        println!("{}", pink_msg.truecolor(255, 128, 171));
    }

    if let Ok(current_exe) = env::current_exe() {
        if let Some(bin_dir) = current_exe.parent() {
            let lmerge_path = bin_dir.join("lmerge");
            
            if lmerge_path.is_file() {
                let path_c = CString::new(lmerge_path.as_os_str().as_bytes()).unwrap();
                let mut args_c: Vec<CString> = vec![CString::new("lmerge").unwrap()];
    ...
    eprintln!("Ошибка: Бинарник lmerge не найден в директории release.");
    Ok(1)
}
...
fn emerge_main(args: &[String]) -> Result<i32, PortageError> {
    ...
    // --- ПЕРЕХВАТ СИСТЕМНЫХ КОМАНД И СЕТОВ ДЛЯ LMERGE (ЗАМЕЩЕНИЕ ПАМЯТИ) ---
    ...
    if has_world {
        return pass_to_lmerge(args, " Передача управления lmerge через execv (Замещение памяти)...");
    }
    if has_search {
        return pass_to_lmerge(args, " Поиск пакета в дереве Portage...");
    }
    if has_sync {
        return pass_to_lmerge(args, ""); // emaint/depclean пробрасываем молча
    }

    // --- ПАРСИНГ ЛОКАЛЬНЫХ АРГУМЕНТОВ ---
    ...
    if packages.is_empty() {
        let needs_packages = options.iter().any(|&opt| 
            opt.starts_with("--oneshot") || opt == "-o" || 
            opt.starts_with("--nodeps") || 
            opt.contains('a') || opt.contains('p') || opt.contains('v')
        );
        if needs_packages {
    ...
    if !packages.is_empty() {
        let is_verbose = options.iter().any(|&opt| opt.contains('v') || opt == "--verbose");
        let is_ask = options.iter().any(|&opt| opt.contains('a') || opt == "--ask");
        let is_pretend = options.iter().any(|&opt| opt.contains('p') || opt == "--pretend");
        let is_unmerge = options.iter().any(|&opt| opt.contains('C') || opt == "--unmerge");
        ...
        // --- УСТАНОВКА ОДИНОЧНЫХ ПАКЕТОВ (КЛАССИЧЕСКИЙ EBUILD) ---
        if is_verbose || is_pretend || is_ask {
...
fn main() {
    ...
    if args[0] == "-wpl" || args[0]  == "--writer-pc-world" {
        ...
        // 3.92 Извлечение и установка шрифтов из крейтов: -wf
    if args[0] == "-wf" || args[0] == "--writer-font" {
        let status = Command::new("writer-font")
            .args(&args[1..]) 
            .status()
            .unwrap_or_else(|_| exit(1));
        exit(status.code().unwrap_or(0));
       ...
       //3.93 Лёгкое emerge
     if args[0] == "-l" || args[0] == "--lmerge" {
        let status = Command::new("lmerge")
            .args(&args[1..]) 
            .status()
            .unwrap_or_else(|_| exit(1));
        exit(status.code().unwrap_or(0));
...
fn main() -> io::Result<()> {
    ...
    let _ = Command::new("groupadd").arg("-r").arg("pkgrs").status();
    ...
    if !status.success() {
        println!("{} {}", "!!!".yellow(), "Пользователь pkgrs уже существует или создан автоматически.");
    }
