...
[[bin]]
name = "writer-git-plus"
path = "src/bin/writer-git-plus.rs"
...
[[bin]]
name = "writer-lib-plus"
path = "src/bin/writer-lib-plus.rs"
 
[[bin]]
name = "writer-bin-plus"
path = "src/bin/writer-bin-plus.rs"

[[bin]]
name = "inspector-rs"
path = "src/bin/inspector-rs.rs"
...
fn main() {
    ...
    if first_arg == "-wg+" || first_arg == "--writer-git-plus" {
        let status = Command::new("writer-git-plus").args(&args[1..]).status().unwrap_or_else(|_| std::process::exit(1));
        std::process::exit(status.code().unwrap_or(0));
    }
    ...
    if first_arg == "-l" || first_arg == "--lmerge" {
     ...
     if first_arg == "-i" || first_arg == "--incpector" {
        let status = Command::new("inspector-rs").args(&args[1..]).status().unwrap_or_else(|_| std::process::exit(1));
        std::process::exit(status.code().unwrap_or(0));
    }
...
fn my_custom_logic() {
	println!("{}", "PKGRS — СИСТЕМА УПРАВЛЕНИЯ ПАКЕТАМИ (RUST + GENTOO)".bold().green());
        ...
        println!(" Для установки с github /usr/bin '.so'");
        println!(" В <url> введите полный адрес github:");println!("  {}", "-wg+, --writer-git-plus: <pkg> <url>".green());
        println!("\n{}", "Пример установки katana :
        pkgrs -wg+ kanata https://github.com/jtroo/kanata.git
        pkgrs -l --ask kanata".blue());
        ...
        println!("\n{}", "Для записи файлов с символом +:".red());
        println!("  {}", "-wl+, --writer-lib-plus: <pkg> <version> (обрезазаем все после плюса и его)".green());
        println!("\n{}", "Шрифты:".yellow());
        ...
        println!("\n{}", "Автоматическая установка из крэйта:".yellow());
        println!("  {}", "pkgrs -i (--inspector) <пакет>".green());
...
fn main() {
    ...
    let _ = io::stdout().flush();
     ...
     ("sys-apps/pkgrs/files/src/bin/rust-openrc-helper.rs", r##"use std::{
    env, fs, 
    io::{Read},
...
fn main() -> std::io::Result<()> {
    ...
    // =========================================================================
    // ПРАВА И ГЕНЕРАЦИЯ: ОРИГИНАЛЬНЫЙ ПОРЯДОК
    // =========================================================================
    let _ = Command::new("chown").args(["-R", "portage:portage", &pkg_dir]).status();
    let _ = Command::new("chmod").args(["-R", "go+rX", &pkg_dir]).status();
    
    println!("{} Запуск автономной манифестации (repoman-rs)...", ">>>".green());
    let path_arg = format!("--path={}", pkg_dir);
    let _ = Command::new("repoman-rs").arg(path_arg).status();
    println!("{} Готово. pkgrs -l -av {}/{}", ">>>".green(), CATEGORY, name);
...
{}{}
 ...
 "##),
    ("sys-apps/pkgrs/files/src/bin/writer-bin-plus.rs", r##"// Copyright 2006-2026 Gentoo Authors
// Distributed under the terms of the GNU General Public License v2
...
fn main() -> std::io::Result<()> {
    ...
    // МЕНЯЕМ ВЕРСИЮ НА ПРАВИЛЬНЫЙ LIVE 9999
    let version = "9999";
    ...
    println!("{} Инициализация проекта-обертки (Bin-Live v{}) через writer-bin-plus...", ">>>".green(), version);

    // Твой бронебойный метод скачивания
    let dl = Command::new("cargo")
        .args(["download", "-x", &format!("{}=={}", name, raw_version)])
        .current_dir(&sources_dir)
        .status();
    ...
    // ОРИГИНАЛЬНЫЙ СИНТАКСИС ШАБЛОНА БЕЗ ИЗМЕНЕНИЙ
    writeln!(f, "#writer-bin")?;
    writeln!(f, "EAPI=8\ninherit cargo\n\nDESCRIPTION=\"{}\"\nHOMEPAGE=\"https://crates.io{}\"\nSRC_URI=\"\"\nS=\"${{WORKDIR}}/{}-{}\"", name, name, name, version)?;
    writeln!(f, "LICENSE=\"MIT\"\nSLOT=\"0\"\nKEYWORDS=\"~amd64\"")?;
    writeln!(f, "IUSE=\"{}\"", iuse_str)?;
...
{}{}
    ...
    ("sys-apps/pkgrs/files/src/bin/writer-git.rs", r##"// Copyright 2006-2026 Gentoo Authors
// Distributed under the terms of the GNU General Public License v2
...
fn main() -> std::io::Result<()> {
    ...
    writeln!(f, "BDEPEND=\"virtual/pkgconfig\"")?;
       ...
       // =========================================================================
    // ИСПРАВЛЕННЫЙ, НЕУБИВАЕМЫЙ ПАТЧ ДЛЯ ЧИСТЫХ БИНАРНИКОВ И БИБЛИОТЕК
    // =========================================================================
    writeln!(f, "\nsrc_prepare() {{
    default
    if [[ -f \"Cargo.toml\" ]]; then
        # КРИТИЧЕСКИЙ МОМЕНТ: Инжектируем [lib] только если в проекте РЕАЛЬНО есть библиотека src/lib.rs!
        if [[ -f \"src/lib.rs\" ]]; then
            einfo \"Обнаружена библиотека. Внимательный патч Cargo.toml для {0}...\"
            ...
            einfo \"Проект {0} распознан как чистый бинарник. Пропускаем патч [lib].\"
        fi
    fi
}}", name, lib_name)?;
    ...
    ("sys-apps/pkgrs/files/src/bin/writer-git-plus.rs", r##"use std::fs;
use std::io::Write;
use std::process::Command;
use colored::*;
...
fn main() -> std::io::Result<()> {
    ...
    println!("{} Готово! Пакет {} из Git настроен.", ">>>".green(), name);
    println!("{} pkgrs -l -av {}/{}", ">>>".green(), CATEGORY, name);
    Ok(())
}
 "##),
...
directory = '../vendor'\" > .cargo/config.toml || die
 ...
 "##),
    ("sys-apps/pkgrs/files/src/bin/writer-lib-plus.rs", r##"// Copyright 2006-2026 Gentoo Authors
// Distributed under the terms of the GNU General Public License v2
...
fn main() -> std::io::Result<()> {
    ...
    // --- ПЕРЕВОД НА 9999 СТРУКТУРУ ---
    // Сохраняем raw_version для Cargo, но для всей системы Gentoo жестко ставим 9999
    let raw_version = &args[2]; 
    let version = "9999";
    // ---------------------------------
    ...
    println!("{} Инициализация проекта-обертки (Live-версия 9999)...", ">>>".green());

    // Создаем build-root через cargo
    Command::new("cargo").args(["new", "--lib", "build-root"]).current_dir(&sources_dir).status()?;
    
    // 2. Добавляем зависимость — ИСПОЛЬЗУЕМ СЫРУЮ ВЕРСИЮ ИЗ АРГУМЕНТОВ
    Command::new("cargo").args(["add", &format!("{}@{}", name, raw_version)]).current_dir(&work_dir).status()?;
    ...
    // Генерируем ebuild с номером 9999
    let ebuild_path = format!("{}/{}-{}.ebuild", pkg_dir, name, version);
    let mut f = fs::File::create(&ebuild_path)?;
    writeln!(f, "#writer-lib-plus")?;
    writeln!(f, "EAPI=8\ninherit cargo\n\nDESCRIPTION=\"{} (wrapped lib live)\"\nHOMEPAGE=\"https://crates.io{}\"\nSRC_URI=\"\"\nS=\"${{WORKDIR}}/build-root\"", name, name)?;
    writeln!(f, "LICENSE=\"MIT\"\nSLOT=\"0\"\nKEYWORDS=\"~amd64\"\nRDEPEND=\"\n\t{}\"\nDEPEND=\"${{RDEPEND}}\"\nBDEPEND=\"virtual/pkgconfig\"", rdepend)?;

    writeln!(f, "\nsrc_unpack() {{ cp -Rp \"{}/\"* \"${{WORKDIR}}/\" || die; }}", sources_dir)?;
    
    // ВЕРНУЛИ ИСХОДНЫЙ СИНТАКСИС ШАБЛОНА НА МЕСТО ОДИН В ОДИН
    writeln!(f, "\nsrc_compile() {{
    # Создаем директорию для конфига
    mkdir -p .cargo || die
...
directory = '../vendor'\" > .cargo/config.toml || die
    ...
    // Финальный вывод ссылается на стабильный live-атом 9999
    println!("{} Готово. pkgrs -l -av {}/={}-{}", ">>>".green(), CATEGORY, name, version);
    Ok(())
}
 "##),
...
fn main() -> std::io::Result<()> {
    let args: Vec<String> = std::env::args().collect();
    if args.len() < 3 {
        println!("Usage: {} <pkg> <version>", "pkgrs -ws".cyan());
        return Ok(());
    }
    let name = &args[1];
