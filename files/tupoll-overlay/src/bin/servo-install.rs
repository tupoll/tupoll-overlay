...
fn main() {
    ...
    app.connect_activate(move |app| {
        ...
        ("www-client/servomenu/files/servomenu/src/bin/grab.rs", r#"use std::env;
use std::fs;
use std::process::Command;
// Используем нашу нативную .so библиотеку, собранную через pkgrs
use slurp_rs::SelectOptions;

fn main() {
    let home = env::var("HOME").expect("HOME not found");
    
    // Точное попадание в официальный кэш Servo с каноничным слэшем
    let cache_dir = format!("{}/.config/servo/cache", home);
    fs::create_dir_all(&cache_dir).ok();

    println!(">>> [Pinnacle-Servo] Ожидание захвата региона для кэша Servo...");

    // 1. Нативно перехватываем регион мыши через библиотеку slurp-rs
    let selection = match slurp_rs::select_region(SelectOptions::default()) {
        Ok(geo) => geo,
        Err(_) => {
            eprintln!(" !!! Отмена: Выделение региона для Servo отменено.");
            std::process::exit(0);
        }
    };

    // Вытаскиваем координаты из внутренней субструктуры .rect
    let geometry = format!(
        "{},{} {}x{}",
        selection.rect.x, selection.rect.y, selection.rect.width, selection.rect.height
    );

    // Защита от нулевого клика
    if selection.rect.width == 0 || selection.rect.height == 0 {
        eprintln!(" !!! Отмена: Нулевой регион захвата.");
        std::process::exit(0);
    }

    // 2. Генерируем каноничное имя файла по маске даты прямо в Rust
    let date_str = Command::new("date")
        .arg("+%Y:%d:%m-%H:%M-page.png")
        .output()
        .map(|o| String::from_utf8_lossy(&o.stdout).trim().to_string())
        .unwrap_or_else(|_| "page.png".to_string());

    let final_filepath = format!("{}/{}", cache_dir, date_str);
    println!(" [+] Регион страницы: {}. Запись в кэш Servo...", geometry);

    // 3. Вызываем wayshot в идеальном синтаксическом порядке под новый парсер
    let _ = Command::new("wayshot")
        .envs(env::vars())
        .arg("-g")
        .arg(&geometry)       // Чистые координаты: "x,y widthxheight"
        .arg("-c")            // Включаем курсор (чтобы видеть клики на странице)
        .arg(&final_filepath) // Путь идет строго в самый конец как аргумент [FILE]
        .status();

    println!(" [+] Срез страницы успешно сохранен в кэш Servo: {}", final_filepath);
} "#),
